...
void send(int socket, vector<char> data)
{
	//const char* dataptr = data.data();
	send(socket,(const char*)data.data(),data.size(),0);
}

void recv(int socket, vector<char>* data)
{
	//dont use
    int dataSize = 0;
    recv(socket, &dataSize, sizeof(int), 0); 

    if (dataSize > 0) {
        data->resize(dataSize);
        recv(socket, data->data(), dataSize, 0); 
    }
}
